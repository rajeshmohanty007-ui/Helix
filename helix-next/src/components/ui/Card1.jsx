...
const Card1 = ({ title, value, icon, growth }) => {
  return (
    <div className="group relative overflow-hidden rounded-3xl border border-(--border-color) bg-(--bg-card) p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
      ...
      {/* Glow Effect */}
      <div className="absolute inset-0 bg-gradient-to-br from-(--accent)/10 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
      ...
      <div className="relative flex items-center justify-between">
        ...
        {/* Right Icon */}
        <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-violet-100 text-(--accent) transition-all duration-300 group-hover:scale-110">
          {icon}
        </div>
      </div>
